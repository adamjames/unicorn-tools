...
-- Whole-frame render API (called once per frame, returns framebuffers)
function render_frame(width, height, t, frame, dt)
...
local fps_start_time = nil
local fps_report_interval = 60  -- Report every N frames

local original_render_frame = render_frame
function render_frame(width, height, t, frame, dt)
    if fps_start_time == nil then
        fps_start_time = t
    end
    fps_frame_count = fps_frame_count + 1
    
    if fps_frame_count % fps_report_interval == 0 then
        local elapsed = t - fps_start_time
        if elapsed > 0 then
            local fps = fps_frame_count / elapsed
            print(string.format("FPS: %.1f  (frame %d, %.2fs)", fps, fps_frame_count, elapsed))
        end
    end
    
    return original_render_frame(width, height, t, frame, dt)
end
